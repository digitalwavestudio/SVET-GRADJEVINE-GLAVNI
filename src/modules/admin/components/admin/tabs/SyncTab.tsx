...
export function SyncTab() {
  ...
  const {
    ...
    queryFn: async () => {
      const data = await apiClient.get<any>("/admin/circuit-breakers");
      const algoliaSync = Array.isArray(data)
        ? data.find((c) => c.name === "AlgoliaSync")
        : null;
      return algoliaSync || { state: "UNKNOWN", failureCount: 0 };
    },
    staleTime: 5 * 60 * 1000, // 5 minuta
  });

  const { data: monitoringMetrics, isLoading: loadingMetrics } = useQuery<any>({
    queryKey: queryKeys.admin.monitoringMetrics,
    queryFn: () => apiClient.get<any>("/admin/monitoring"),
    staleTime: 5 * 60 * 1000, // 5 minuta
  });
  ...
  const handleRetryBulkDlq = async () => {
    if (
      !window.confirm(
        "Da li ste sigurni da želite da ponovite SVE neuspele DLQ zadatke?",
      )
    )
      return;
    try {
      const { retriedCount } = await apiClient.post<{ retriedCount: number }>(
        "/admin/dlq/retry",
        {},
      );
      alert(
        `Uspešno poslato ${retriedCount} zadataka na ponovno procesuiranje.`,
      );
      fetchDlqItems();
    } catch (err) {
      console.error(err);
      alert("Došlo je do greške prilikom ponovnog pokušaja svih zadataka.");
    }
  };
  ...
  const handleRetryDlq = async (id: string, source: string) => {
    if (
      !window.confirm(
        "Da li ste sigurni da želite da pokušate ponovo ovaj zadatak?",
      )
    )
      return;
    try {
      await apiClient.post(`/admin/dlq/${id}/retry`, { source });
      alert("Zadatak uspešno poslat na ponovno procesuiranje.");
      fetchDlqItems();
    } catch (err) {
      console.error(err);
      alert("Došlo je do greške prilikom ponovnog pokušaja.");
    }
  };

  if (loading && !syncStatus && !monitoringMetrics)
    return <Skeleton className="w-full h-80 rounded-[10px]" />;
  ...
  const handleReindex = async () => {
    if (
      !window.confirm(
        "Da li ste sigurni da želite da pokrenete Re-index all proces? Ovo može trajati neko vreme.",
      )
    )
      return;
    setReindexing(true);
    try {
      ...
      alert("Re-index proces uspešno pokrenut u pozadini.");
    } catch (err) {
      console.error(err);
      alert("Došlo je do greške prilikom re-indeksiranja.");
    } finally {
      setReindexing(false);
      fetchSyncStatus();
    }
    ...
    >
      <div className="flex flex-col gap-2">
        <h2 className="text-4xl font-black text-white uppercase tracking-tighter">
          SISTEMSKI MONITOR & SINHRONIZACIJA
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="p-8 rounded-[10px] border relative overflow-hidden bg-white/5 border-white/10 flex flex-col justify-between">
            ...
            <div className="flex flex-col gap-4 mt-8">
              <div className="flex justify-between items-center bg-black/40 p-4 rounded-[10px] border border-white/5">
                ...
                <span className="text-2xl font-black text-secondary">
                  {monitoringMetrics?.avgResponseTime || 0} ms
                </span>
              </div>
              <div className="flex justify-between items-center bg-black/40 p-4 rounded-[10px] border border-white/5">
                <span className="text-white/40 font-bold uppercase text-xs tracking-widest">
              ...
              <div className="flex justify-between items-center bg-black/40 p-4 rounded-[10px] border border-white/5">
                ...
                <span className="text-xl font-black text-white/70">
                  {(monitoringMetrics?.instanceUptimeSeconds || 0) / 3600 > 1
                    ? `${Math.floor((monitoringMetrics?.instanceUptimeSeconds || 0) / 3600)}h`
                    : `${Math.floor((monitoringMetrics?.instanceUptimeSeconds || 0) / 60)}m`}
                </span>
              </div>
              <div className="flex justify-between items-center bg-black/40 p-4 rounded-[10px] border border-white/5">
                <span className="text-white/40 font-bold uppercase text-xs tracking-widest">
                ...
                <span
                  className={`text-2xl font-black ${(monitoringMetrics?.outbox?.pending || 0) > 50 ? "text-amber-500" : "text-green-500"}`}
                >
                  {monitoringMetrics?.outbox?.pending || 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8">
            <h4 className="text-xs font-bold text-red-400 uppercase tracking-widest mb-4">
              Rate limit i sistemske greške (poslednjih 50)
            </h4>
            <div className="bg-black/60 rounded-[10px] p-4 max-h-[150px] overflow-y-auto border border-red-500/10 font-mono text-[10px] text-red-300">
              {monitoringMetrics?.errors?.length > 0 ? (
                monitoringMetrics.errors.map((err: string, i: number) => (
                  <div
                    key={i}
                    className="mb-2 border-b border-red-500/10 pb-2 last:mb-0 last:border-0 last:pb-0"
                  >
                    {err}
                  </div>
                ))
                ...
                <span className="text-green-500">
                  Svež start, nema grešaka ni Rate Limit blokada.
                </span>
              )}
            </div>
          </div>
        ...
        {/* Algolia Sync Box */}
        <div
          className={`p-8 rounded-[10px] border relative overflow-hidden flex flex-col justify-between ${isHealthy ? "bg-green-500/10 border-green-500/20" : "bg-red-500/10 border-red-500/20"}`}
        >
          <div className="relative z-10 flex flex-col gap-4">
            <div
              className={`w-16 h-16 rounded-[10px] flex items-center justify-center ${isHealthy ? "bg-green-500/20 text-green-500" : "bg-red-500/20 text-red-500"}`}
            >
            ...
            <div>
              <h3 className="text-xl font-black text-white uppercase tracking-tight">
                AUTOMATSKA SINHRONIZACIJA AKTIVNA
              </h3>
              <p
                className={`text-xs font-bold uppercase tracking-widest mt-1 ${isHealthy ? "text-green-500" : "text-red-500"}`}
              >
                {isHealthy
                  ? "EKSTERNI PRETRAŽIVAČ RADI U REALNOM VREMENU"
                  : "OTKRIVENI SU PROBLEMI!"}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-6 p-6 bg-black/40 rounded-[10px] border border-white/5">
              <div className="flex justify-between items-center text-sm">
                <span className="text-white/40 font-bold uppercase">
                  Neuspešni mrežni pokušaji (Failures)
                </span>
              ...
              <div className="flex justify-between items-center text-sm mt-3 pt-3 border-t border-white/5">
                <span className="text-white/40 font-bold uppercase">
                  Stanje zaštitnika (Circuit Breaker)
                </span>
                <span className="text-white font-black">
                  {syncStatus?.state || "N/A"}
                </span>
              </div>
              <div className="flex justify-between items-center text-sm mt-3 pt-3 border-t border-white/5">
                <span className="text-white/40 font-bold uppercase">
                  Uspeli Sync Taskovi (Cache/Redis)
                </span>
                <span className="text-green-400 font-black">
                  {monitoringMetrics?.syncSuccess || 0}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-b border-white/5 pb-4">
            <div>
              ...
              <p className="text-xs font-bold text-white/40 uppercase tracking-widest mt-1">
                ZADACI KOJI SU PALI VIŠE OD 5 PUTA
              </p>
            </div>
              ...
              <tbody>
                {dlqItems.map((doc) => (
                  <React.Fragment key={doc.id}>
                    <tr
                      ...
                      onClick={() =>
                        setExpandedJob(expandedJob === doc.id ? null : doc.id)
                      }
                    >
                      <td className="p-4">
                        <span
                          className={`px-2 py-1 rounded-[4px] text-[10px] font-black uppercase ${doc.source === "outbox" ? "bg-blue-500/20 text-blue-400" : "bg-amber-500/20 text-amber-400"}`}
                        >
                          {doc.source}
                        </span>
                      </td>
                      ...
                      <td className="p-4">
                        <p className="text-xs text-red-400 max-w-[300px] truncate">
                          {doc.error}
                        </p>
                      </td>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2">
                              ...
                              <div className="p-4 bg-black/60 rounded-[10px] border border-red-500/20 font-mono text-xs text-red-300 whitespace-pre-wrap">
                                {doc.error}
                                {doc.stack && (
                                  <div className="mt-4 opacity-50 border-t border-red-500/10 pt-4 text-[10px]">
                                    {doc.stack}
                                  </div>
                                )}
                              </div>
                ...
                {dlqItems.length === 0 && (
                  <tr>
                    <td
                      colSpan={5}
                      className="p-8 text-center text-white/40 font-medium"
                    >
                      Nema neuspelih zadataka u DLQ-u.
                    </td>
                  </tr>
                )}
              </tbody>
