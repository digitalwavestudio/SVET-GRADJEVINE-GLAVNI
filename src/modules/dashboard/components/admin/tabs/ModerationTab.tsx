...
export function ModerationTab({ getDetailLink }: ModerationTabProps) {
  ...
  } = useAdminModeration(debouncedQuery);
  ...
  const handleGlobalApprove = async (collName: string, id: string, itemData?: ModerationItem) => {
    if (!navigator.onLine) {
      toast.error("Niste povezani na mrežu. Sve akcije odobravanja i odbijanja oglasa su privremeno onemogućene kada ste offline.");
      return;
    }
    try {
      const auth = getAuth();
      ...
      toast.error(err.message || "Greška pri odobravanju");
    }
  };
  ...
  const handleGlobalReject = async (collName: string, id: string) => {
    ...
    try {
      ...
      toast.error(err.message || "Greška pri odbijanju oglasa");
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (!navigator.onLine) {
      toast.error("Niste povezani na mrežu. Izmena i čuvanje oglasa su onemogućeni dok ste van mreže.");
      return;
    }
    
    // Zod validation on client
    if (!editTitle || editTitle.trim().length < 3) {
      toast.error('Naslov mora imati bar 3 karaktera.');
      return;
    }
