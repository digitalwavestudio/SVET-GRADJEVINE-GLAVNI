...
const normalizeCompany = (c: any): Company => {
  ...
  return {
    ...
    logo: c.logo || c.businessProfile?.logo || (c.images && c.images.length > 0 ? c.images[0] : undefined) || null,
    isVerified: c.status === 'active' || c.isVerified,
    phone: c.phone || c.businessProfile?.phone || '',
    website: c.website || c.companyWeb || c.businessProfile?.website || '',
    facebook: c.facebook || c.companyFB || '',
    instagram: c.instagram || c.companyIG || '',
    locationSlug: c.locationSlug || c.location || '',
    email: c.email || '',
    coverImage: c.coverImage || c.businessProfile?.coverImage || undefined,
