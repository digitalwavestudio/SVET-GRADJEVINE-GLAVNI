...
export const emailService = {
   ...
   */
  async sendJobApplicationNotification(employerEmail: string, jobTitle: string, applicantName: string) {
    const html = `
    ...
    `;
    return this.sendEmail({ to: employerEmail, subject: `Nova prijava: ${jobTitle}`, html });
  },

  /**
   * Šablon: Potvrda verifikacije (Plava kvačica)
   */
  async sendVerificationApprovedNotification(userEmail: string, userName: string) {
    const html = `
    ...
    `;
    return this.sendEmail({ to: userEmail, subject: `Profil Verifikovan - Svet Građevine`, html });
  },
  ...
  async sendAdApprovedNotification(userEmail: string, adTitle: string) {
    ...
    `;
    return this.sendEmail({ to: userEmail, subject: `Oglas odobren: ${adTitle}`, html });
  },

  /**
   * Šablon: Oglas odbijen
   */
  async sendAdRejectedNotification(userEmail: string, adTitle: string, reason: string) {
    const html = `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee;">
    ...
    `;
    return this.sendEmail({ to: userEmail, subject: `Obaveštenje o oglasu: ${adTitle}`, html });
  },
  ...
  async sendRfqNotification({ to, category, region, phone, specification }: { to: string; category: string; region: string; phone: string; specification: Array<{ name: string; amount: number | string; unit: string; desc?: string }> }) {
    ...
    `;
    return this.sendEmail({ to, subject: `[B2B RFQ Tender] Potražnja za materijalom - ${category} (${region})`, html });
  },
};
