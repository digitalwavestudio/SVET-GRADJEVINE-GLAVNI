...
export class NotificationService {
  ...
  static async send(payload: NotificationPayload) {
    try {
      ...
      await CacheService.set(`notifications_pending_count:${payload.userId}`, "1", 30 * 24 * 60 * 60 * 1000).catch((e: any) => NotificationService.logger.warn("[NotificationService] Cache set pending count:", e));
